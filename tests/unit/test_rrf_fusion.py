...
def test_reciprocal_rank_fusion_rewards_consistent_top_ranking():
    ...
    list_b = [
        {"id": "x", "distance": 0.9},
        {"id": "z", "distance": 0.95},
    ]  # different scale, x still ranked #1
    fused = _reciprocal_rank_fusion([list_a, list_b])
    assert (
        fused[0]["id"] == "x"
    ), "item ranked #1 in both queries should win the fusion regardless of raw distance scale"
...
def test_fusion_not_dominated_by_one_querys_distance_scale():
    ...
    list_b = [
        {"id": "wrong1", "distance": 0.10},
        {"id": "wrong2", "distance": 0.11},
    ]  # wrong, but "good" distances
    fused = _reciprocal_rank_fusion([list_a, list_b])
    # rank-based: "correct" is rank 1 in its list (1/61), "wrong1" is too — tie by rank
