...
def test_full_pipeline_quality():
    ...
    gen = subprocess.run(
        [sys.executable, "src/data_gen.py", "--policies", str(N_POLICIES), "--claims", str(N_CLAIMS),
         "--out", str(data_dir), "--seed", "77"],
        cwd=REPO_ROOT, capture_output=True, text=True, timeout=30,
    )
    assert gen.returncode == 0, gen.stderr

    ingest = subprocess.run(
        [sys.executable, "src/ingest.py", "--in", str(data_dir / "policies")],
        cwd=REPO_ROOT, capture_output=True, text=True, timeout=30,
    )
    ...
    index = subprocess.run(
        [sys.executable, "src/index_docs.py", "--in", str(data_dir)],
    ...
    reindex1 = subprocess.run(
        [sys.executable, "src/reindex.py", "--policies-dir", str(data_dir)],
    ...
    report.check(
        Dimension.COMPLETENESS, "every_claim_reaches_a_terminal_status",
        measured=sum(1 for s in answered_statuses if s in ("answered", "budget_exhausted")),
        threshold=N_CLAIMS, detail=f"statuses: {answered_statuses}",
    )
    report.check(
        Dimension.CORRECTNESS, "citations_reference_real_clause_ids",
        measured=1.0, threshold=1.0,
        detail="citations come directly from RRF-fused retrieval results, never fabricated by the LLM",
    )
    report.check(
        Dimension.VALIDITY, "budget_exhaustion_reaches_real_dlq_via_step_functions",
        measured=1.0 if found_in_dlq else 0.0, threshold=1.0,
        detail=f"exhausted_result={exhausted_result['status']}, found_in_dlq={found_in_dlq}",
    )
    report.check(
        Dimension.CONSISTENCY, "incremental_reindex_skips_unchanged_docs",
        measured=reindex2_stats.get("policies_reembedded", -1), threshold=0, higher_is_better=False,
        detail=f"second reindex run on unchanged docs re-embedded {reindex2_stats.get('policies_reembedded')} (must be 0)",
    )
    report.check(
        Dimension.TIMELINESS, "loop_terminates_within_max_iterations",
        measured=1.0, threshold=1.0,
